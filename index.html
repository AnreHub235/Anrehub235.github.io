...
function submit() {
  if (url.includes("?api=")) {
    // Api
    if (apiValue === "WinningSmile_018500234_)*%)(!@&%)!_LAHJOIAMSFOCS" && "WinningSmile_09120958_)*!&%)(@**%&!@(*&%_ALOJPOFJVCBOAF" && "WinningSmile_198540129809_()!%(*@&^%$@@!%%_AJUSPOJFXOSFG") {
...
async function codex() {
  ...
  async function e() {
    ...
    key = "Error: Failed to get stages"
  }
  async function a(e) {
    let a = await (
    ...
    key =  "Error: Failed to initiate stage"
  }
  async function o(e, a) {
    ...
    key = "Error: Failed to validate stage"
  }
  async function n(e) {
    if (
      (
      ...
      return !0;
    key =  "Error: Failed to authenticate"
  }
  function i(t) {
    let e = t.split(".")[1];
...
!(function () {
  ...
  if (
    ...
    if (e.includes("sub2unlock.com/link/unlock")) {
      ...
      let o = e.split("/"),
        n = o[o.length - 1];
      key =  "https://sub2unlock.com/link/unlock/" + n;
    }
    ...
    try {
      ...
      if (i) {
        ...
        s.props.pageProps.data.targeturl
          ? key =  s.props.pageProps.data.targeturl
          : window.location.reload();
  ...
  start();
    ...
    await browser.close();
      ...
      document.write('<pre style="word-wrap: break-word; white-space: pre-wrap;">{"key":"Invalid API, please join https://discord.gg/uq8V2y7Y4k find user @Winning Smile .Maybe him giveaway"}</pre>');
    }
    // No FoundAPI
    } else {
      document.write('<pre style="word-wrap: break-word; white-space: pre-wrap;">{"key":"Not Found API ?, please join https://discord.gg/uq8V2y7Y4k find user @Winning Smile .Maybe him giveaway"}</pre>');
    }
}
